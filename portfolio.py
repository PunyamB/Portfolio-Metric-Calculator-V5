...
def calculate_current_capital(portfolio_market_value):
    ...
    buys = trades.loc[trades["TransactionType"] == "Buy", "Amount"].sum()
    sells = trades.loc[trades["TransactionType"] == "Sell", "Amount"].sum()
    shorts = trades.loc[trades["TransactionType"] == "Short", "Amount"].sum()

    cash_remaining = STARTING_CAPITAL - buys + sells + shorts
