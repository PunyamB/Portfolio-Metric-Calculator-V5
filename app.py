...
""")
    ...
    st.markdown("""
A read-only view of all imported trades with filters by transaction type, symbol, and security type. Summary cards show total bought, total sold/shorted, net cash flow, and realised P&L.
""")
...
with tab_portfolio:
    ...
    if portfolio.empty:
        ...
        with sub_metrics:
            ...
            with col_btn:
                lookback_label_height = "Lookback Period"
                st.markdown(f"<p style='margin-bottom:0; font-size:14px; color:transparent;'>{lookback_label_height}</p>", unsafe_allow_html=True)
...
with tab_history:
    ...
    if trade_history.empty:
        ...
        column_config={
            ...
            "Price": st.column_config.NumberColumn("Price", width="medium"),
            "Amount": st.column_config.NumberColumn("Amount", width="medium"),
        })
