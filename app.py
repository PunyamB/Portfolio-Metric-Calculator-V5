...
with tab_reports:
    ...
    st.caption("Generate professional weekly or inception-to-date reports with AI-powered narrative.")
    ...
    with rep_col1:
        ...
        st.caption("Compares this week's Friday close to prior Friday. Handles mid-week trades automatically.")
        week_date = st.date_input("Week ending (Friday)", value=None, key="report_week_date")
        if st.button("Generate Weekly Report", type="primary", key="gen_weekly_btn", disabled=not gemini_key):
            with st.spinner("Reconstructing portfolios, computing metrics, generating narrative..."):
                try:
                    from report_data import compute_weekly_report_data
                    from report_narrative import generate_weekly_narrative
                    from report_gen import generate_weekly_report
                    import os

                    report_data = compute_weekly_report_data(week_date)
                    if "error" in report_data:
                        st.error(report_data["error"])
                    else:
                        narrative = generate_weekly_narrative(report_data, gemini_key)
                        output_path = os.path.join(os.getcwd(), "weekly_report.docx")
                        generate_weekly_report(report_data, narrative, output_path)
                        with open(output_path, "rb") as f:
                            st.download_button(
                                label="Download Weekly Report",
                                data=f.read(),
                                file_name=f"weekly_report_{report_data['period_end']}.docx",
                                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                key="dl_weekly"
                            )
                        st.session_state["weekly_report_data"] = report_data
                        st.session_state["weekly_narrative"] = narrative
                except Exception as e:
                    st.error(f"Report generation failed: {str(e)}")

    with rep_col2:
        st.markdown("**Inception Report**")
        st.caption("Full portfolio analysis from January 26, 2026 to current. Includes realised P&L from closed positions.")
        if st.button("Generate Inception Report", type="primary", key="gen_inception_btn", disabled=not gemini_key):
            with st.spinner("Reconstructing full history, computing metrics, generating narrative..."):
                try:
                    from report_data import compute_inception_report_data
                    from report_narrative import generate_inception_narrative
                    from report_gen import generate_inception_report
                    import os

                    report_data = compute_inception_report_data()
                    if "error" in report_data:
                        st.error(report_data["error"])
                    else:
                        narrative = generate_inception_narrative(report_data, gemini_key)
                        output_path = os.path.join(os.getcwd(), "inception_report.docx")
                        generate_inception_report(report_data, narrative, output_path)
                        with open(output_path, "rb") as f:
                            st.download_button(
                                label="Download Inception Report",
                                data=f.read(),
                                file_name=f"inception_report_{report_data['period_end']}.docx",
                                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                key="dl_inception"
                            )
                        st.session_state["inception_report_data"] = report_data
                        st.session_state["inception_narrative"] = narrative
                except Exception as e:
                    st.error(f"Report generation failed: {str(e)}")

    # show preview if generated
    if "weekly_narrative" in st.session_state:
        with st.expander("Weekly Report Preview"):
            n = st.session_state["weekly_narrative"]
            d = st.session_state["weekly_report_data"]
            st.caption(f"{d['period_start']} to {d['period_end']}")
            st.markdown(f"**Portfolio:** ${d['prior_value']:,.2f} → ${d['current_value']:,.2f} ({d['value_change_pct']:+.2f}%)")
            st.markdown(f"**Benchmark (S&P 500):** {d['benchmark_weekly_return']:+.2f}%")
            st.markdown("---")
            st.markdown(f"**Executive Summary:** {n.get('executive_summary', 'N/A')}")
            st.markdown(f"**Movers:** {n.get('movers_commentary', 'N/A')}")
            st.markdown(f"**Risk:** {n.get('risk_analysis', 'N/A')}")
            st.markdown(f"**Outlook:** {n.get('outlook', 'N/A')}")

    if "inception_narrative" in st.session_state:
        with st.expander("Inception Report Preview"):
            n = st.session_state["inception_narrative"]
            d = st.session_state["inception_report_data"]
            st.caption(f"{d['period_start']} to {d['period_end']}")
            st.markdown(f"**Total P&L:** ${d.get('total_pnl', 0):,.2f} (Unrealised: ${d.get('unrealised_pnl', 0):,.2f}, Realised: ${d.get('realised_pnl', 0):,.2f})")
            st.markdown(f"**Benchmark (S&P 500):** {d.get('benchmark_return', 0):+.2f}%")
            st.markdown("---")
            st.markdown(f"**Executive Summary:** {n.get('executive_summary', 'N/A')}")
            st.markdown(f"**Attribution:** {n.get('performance_attribution', 'N/A')}")
            st.markdown(f"**Risk:** {n.get('risk_assessment', 'N/A')}")
            st.markdown(f"**Strategy:** {n.get('forward_strategy', 'N/A')}")
...
""")
    ...
    st.markdown("""
A read-only view of all imported trades with filters by transaction type, symbol, and security type. I've included summary cards showing total bought, total sold/shorted, net cash flow, and realised P&L. This tab exists for transparency and audit — since the CSV is the source of truth, I make edits in the file itself.
""")
