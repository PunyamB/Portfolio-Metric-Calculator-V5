...
# --- tabs ---
tab_portfolio, tab_history, tab_simulator = st.tabs(["Portfolio", "Trade History", "What-If Simulator"])
...
with tab_simulator:
    ...
    if portfolio.empty:
        ...
        st.caption("Mean-variance optimization. Finds optimal weight allocation for your selected tickers.")
        ...
        with opt_c2: opt_long_pct = st.number_input("Long %", value=73, min_value=0, max_value=100, step=1, key="opt_long_pct")
        with opt_c3: opt_short_pct = st.number_input("Short %", value=27, min_value=0, max_value=100, step=1, key="opt_short_pct")
        ...
        if opt_long_pct + opt_short_pct > 100:
            st.warning(f"Long + Short = {opt_long_pct + opt_short_pct}%. Exceeds 100%.")
        ...
        if all_opt_tickers:
                ...
                if not opt_long_list:
                    ...
                    with st.spinner("Running Markowitz optimization..."):
                        ...
                        opt_result = run_markowitz_optimization(
                            ...
                            long_pct=opt_long_pct, short_pct=opt_short_pct, max_per_position=opt_max_pos)
            ...
            if "markowitz_results" in st.session_state:
                ...
                mh1, mh2, mh3, mh4 = st.columns([2, 1.5, 1.5, 1.5])
                mh1.markdown("**Metric**"); mh2.markdown("**Current**"); mh3.markdown("**Max Sharpe**"); mh4.markdown("**Min Variance**")
                st.markdown("---")
                for label, key, fmt in comp:
                    mc1, mc2, mc3, mc4 = st.columns([2, 1.5, 1.5, 1.5])
                    mc1.write(label)
                    cv = cur_m.get(key); msv = ms.get(key); mvv = mv.get(key)
                ...
                with at1:
                    ...
                    if not ms["table"].empty:
                        st.dataframe(ms["table"].style.format({
                            "Dollar Amount ($)": "${:,.2f}",
                            "Weight (%)": "{:.2f}",
                        }), use_container_width=True, hide_index=True)
                with at2:
                    ...
                    if not mv["table"].empty:
                        st.dataframe(mv["table"].style.format({
                            "Dollar Amount ($)": "${:,.2f}",
                            "Weight (%)": "{:.2f}",
                        }), use_container_width=True, hide_index=True)
