...
def run_markowitz_optimization(
    ...
    total_capital=1_000_000, long_pct=73, short_pct=27,
    max_per_position=90_000, n_simulations=10000,
):
    ...
    long_alloc = long_pct / 100
    short_alloc = short_pct / 100
    max_w = max_per_position / total_capital if total_capital > 0 else 0.1

    bounds = []
    for i in range(n):
        if i in long_idx:
            bounds.append((0.0, min(max_w, long_alloc)))
        else:
            bounds.append((-min(max_w, short_alloc), 0.0))
    bounds = tuple(bounds)
    ...
    constraints = [
        {"type": "ineq", "fun": lambda w: long_alloc - sum(w[i] for i in long_idx)},
        {"type": "ineq", "fun": lambda w: sum(w[i] for i in long_idx)},
    ]
    if short_idx:
        constraints.extend([
            {"type": "ineq", "fun": lambda w: short_alloc + sum(w[i] for i in short_idx)},
            {"type": "ineq", "fun": lambda w: -sum(w[i] for i in short_idx)},
        ])

    w0 = np.zeros(n)
    init_long_w = min(long_alloc / max(len(long_idx), 1), max_w)
    init_short_w = min(short_alloc / max(len(short_idx), 1), max_w)
    for i in long_idx: w0[i] = init_long_w
    for i in short_idx: w0[i] = -init_short_w

    ms_result = minimize(portfolio_sharpe, w0, method="SLSQP",
                         bounds=bounds, constraints=constraints, options={"maxiter": 1000})
    ms_w = ms_result.x if ms_result.success else w0

    mv_result = minimize(portfolio_volatility, w0, method="SLSQP",
                         bounds=bounds, constraints=constraints, options={"maxiter": 1000})
    mv_w = mv_result.x if mv_result.success else w0

    frontier_returns, frontier_vols, frontier_sharpes = [], [], []
    for _ in range(n_simulations):
        w = np.zeros(n)
        if long_idx:
            raw = np.random.dirichlet(np.ones(len(long_idx))) * long_alloc
            raw = np.minimum(raw, max_w)
            scale = np.random.uniform(0.5, 1.0)
            raw *= scale
            for j, idx in enumerate(long_idx): w[idx] = raw[j]
        if short_idx:
            raw = np.random.dirichlet(np.ones(len(short_idx))) * short_alloc
            raw = np.minimum(raw, max_w)
            scale = np.random.uniform(0.5, 1.0)
            raw *= scale
    ...
    def calc_metrics(w):
        ...
        allocated = sum(abs(w[i]) for i in range(n)) * total_capital
        cash = total_capital - allocated
    ...
    return {
        "tickers": available,
        "max_sharpe": {**calc_metrics(ms_w), "table": build_alloc_table(ms_w), "weights": ms_w},
